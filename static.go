...
func (h *uiAssetsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// Get the absolute urlpath to prevent directory traversal.
	urlpath := path.Clean(r.URL.Path)

	// Get the urlpath relative to the root urlpath.
	if !strings.HasPrefix(urlpath, h.rootPath) {
		http.Error(w, "unexpected urlpath prefix", http.StatusBadRequest)
		return
	}
	urlpath = strings.TrimPrefix(urlpath, h.rootPath)

	if code, err := h.serveFile(w, urlpath); err != nil {
